...
func GetMoBoBySocket(c *gin.Context) {

	moboSocket := c.Param("sockettype")
	var motherBoards []models.MotherBoard
	result := database.DB.Where("socket_type = ?", moboSocket).Find(&motherBoards)

	if result.Error != nil {
		c.JSON(404, gin.H{"error": "Error retrieving MotherBoards by socket"})
		return
	}
